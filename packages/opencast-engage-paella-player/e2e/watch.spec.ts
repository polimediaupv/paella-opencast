...
test.describe('Load', () => {
    test('Has the correct title', async ({ page }) => {
        ...
        await expect(page).toHaveTitle(
            'Strong river flowing down the forest - No series | Opencast',
        );
    });
...
test.describe('Player URL query parameters', () => {
    test('Without additional query parameters', async ({ page }) => {
        ...
        const currentTime = await page.evaluate(
            `${playerInstanceStr}.videoContainer.currentTime()`,
        );
        const captionsVisible = await page.evaluate(
            `${playerInstanceStr}.captionsCanvas.isVisible`,
        );
    ...
    test('Check time param in URL and seek: ?time=10s', async ({ page }) => {
        await page.goto(
            '/paella8/ui/watch.html?id=ID-strong-river-flowing-down-the-green-forest&time=10s',
        );
        await clickToStartVideo(page);
        await pauseVideo(page);
    ...
    test('Check trimming param in URL: ?trimming=5s;20s', async ({ page }) => {
        await page.goto(
            '/paella8/ui/watch.html?id=ID-strong-river-flowing-down-the-green-forest&trimming=5s;20s',
        );
        await clickToStartVideo(page);
        await pauseVideo(page);
        await page.waitForTimeout(5000);
    ...
    test('Check time param in URL and seek: &start=5&end=20', async ({ page }) => {
        await page.goto(
            '/paella8/ui/watch.html?id=ID-strong-river-flowing-down-the-green-forest&start=5&end=20',
        );
        await clickToStartVideo(page);
        await pauseVideo(page);
        await page.waitForTimeout(5000);
