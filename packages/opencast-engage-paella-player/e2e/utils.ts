...
export const getPlayerState = async (page: Page): Promise<Record<string, string>> =>
    await page.evaluate(`${playerInstanceStr}.PlayerState`);

export const waitState = async (page: Page, state: string): Promise<void> => {
    await page.evaluate(`${playerInstanceStr}.waitState(${state})`);
};

export const getState = async (page: Page): Promise<string> =>
    await page.evaluate(`${playerInstanceStr}.state`);
...
export const playVideo = async (page: Page): Promise<void> =>
    await page.evaluate(`${playerInstanceStr}.play()`);
export const pauseVideo = async (page: Page): Promise<void> =>
    await page.evaluate(`${playerInstanceStr}.pause()`);

export const test = base.extend({
    page: async ({ page }, use) => {
        await page.route(
            '**/search/episode.json?id=ID-strong-river-flowing-down-the-green-forest',
            async (route) => {
