...
const ocProxyServer =
    process.env.OC_SERVER_URL ?? process.env.OC_PRESENTATION_URL ?? 'https://stable.opencast.org';
...
export default defineConfig({
    ...
    plugins: [tailwindcss(), expressPlugin()],
    define: {
        'import.meta.env.OC_PRESENTATION_URL': JSON.stringify(
            process.env.OC_PRESENTATION_URL || '/',
        ),
        'import.meta.env.OC_PAELLA8_BASE_URL': JSON.stringify(baseURL),
        'import.meta.env.CONFIG_FOLDER': JSON.stringify(
            process.env.CONFIG_FOLDER || '/ui/config/paella8/',
        ),
        'import.meta.env.OC_PAELLA8_URL_TEMPLATE': JSON.stringify(
            process.env.OC_PAELLA8_URL_TEMPLATE,
        ),
        'import.meta.env.USE_OC_SERVER_FROM_URL': JSON.stringify(
            process.env.USE_OC_SERVER_FROM_URL || 'false',
        ),
    },
    optimizeDeps: {
