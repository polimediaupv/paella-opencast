...
export async function defaultLoadConfigFunc(
    configUrl: string,
    player: Paella,
): Promise<OpencastPaellaConfig> {
    ...
    if (ocInitParams.opencast?.paellaConfig) {
        ...
        if (typeof ocInitParams.opencast.paellaConfig === 'string') {
            try {
                ocPlayer.log.info(
                    'Parsing paella config from string',
        ...
        // If paellaConfig is an object, use it directly.
            ...
            throw new Error(
                'Error getting paella config. paellaConfig is not a string or an Config object.',
            );
        }
    } else {
        ...
        ocPlayer.log.info(
            `Loading paella config from URL: ${configFileUrl}`,
            '@asicupv/paella-opencast-core',
        );
...
export async function defaultGetVideoIdFunc(
    config: OpencastPaellaConfig,
    player: Paella,
): Promise<string | null> {
    const ocPlayer = player as OpencastPaellaPlayer;
    const ocInitParams = ocPlayer.initParams as OpencastInitParams;
    ...
    const videoId =
        ocInitParams.opencast?.videoId ?? getHashOrUrlParameter('id') ?? config?.fallbackId ?? null;
...
export async function defaultLoadVideoManifestFunc(
    _manifestUrl: string,
    _config: OpencastPaellaConfig,
    player: Paella,
): Promise<Manifest> {
    player.log.info(
        '================== loadVideoManifest ==================',
    ...
    if (ocInitParams.opencast?.episode) {
        player.log.info(
            'Loading episode from opencast episode attribute',
            '@asicupv/paella-opencast-core',
        );
        try {
    ...
    // Load episode from videoId. Will autodiscover if it is a serarch API or an External API response.
        ...
        const url = ocPlayer.getUrlFromOpencastServer(
            `/search/episode.json?id=${player.videoId ?? ''}&limit=1`,
        );
        if (url === null) {
            throw new Error(
                'Error getting opencast episode URL. opencast-presentation-url is not set.',
            );
        }
        player.log.info(
            `Fetching opencast episode '${player.videoId}' from '${url}'`,
            '@asicupv/paella-opencast-core',
        );
        try {
            opencastEvent = await fetch(url).then(async (response) => await response.json());
        } catch {
            throw new Error('Error fetching opencast episode');
        }
    }

    const paellaEpisode = await getPaellaManifestFromOpencastEvent(
        ocPlayer,
        opencastEvent,
        conversionConfig,
    );
    if (paellaEpisode === null) {
        ...
        if (userName === null) {
            ocPlayer.log.info(
                'User not authenticated. Redirecting to authentication...',
                '@asicupv/paella-opencast-core',
            );
...
async function getPaellaManifestFromOpencastEvent(
    player: Paella,
    episode: any,
    ...
    if (episode['search-results']) {
        player.log.info(
            'opencastEpisodeToManifest: convert!!!!!!!!!!!!',
            '@asicupv/paella-opencast-core',
        );
    ...
    if (episode?.total === 1 && episode?.result) {
        player.log.info(
            'opencastEpisodeToManifest: Episode is an Opencast search result. Converting to paella manifest!',
            '@asicupv/paella-opencast-core',
        );
    ...
    // Check if the episode is an Opencast external API event
        ...
        player.log.info(
            'opencastEpisodeToManifest: Episode is an Opencast external API Event result. Converting to paella manifest!',
            '@asicupv/paella-opencast-core',
        );
    ...
    player.log.info(
        'opencastEpisodeToManifest: unknown episode format. Cannot convert to paella manifest!',
        '@asicupv/paella-opencast-core',
    );
    return null;
}
