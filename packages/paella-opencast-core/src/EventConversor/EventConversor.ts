...
export class EventConversor {
    ...
    getTimeLineInfo(event: Event): NonNullable<Manifest['metadata']>['timeline'] | undefined {
        ...
        const timelineFlavors = this.conversionConfig.timelineAttachmentsFlavours ?? [
            'presentation/timeline+preview',
            'presenter/timeline+preview',
        ];
        const previewAttachment = attachments.find((attachment) =>
            timelineFlavors.includes(attachment.flavor),
        );

        if (previewAttachment) {
            const rows = previewAttachment?.additionalProperties?.imageSizeX
                ? Number(previewAttachment.additionalProperties.imageSizeX)
                : 10;
            const cols = previewAttachment?.additionalProperties?.imageSizeY
                ? Number(previewAttachment.additionalProperties.imageSizeY)
                : 10;
            return {
                ...
                if (captions_match) {
                    ...
                    if (mpElement.tags) {
                        mpElement.tags.forEach((tag) => {
                            ...
                            if (
                                tag.startsWith('generator-type:') &&
                                tag.substring('generator-type:'.length) === 'auto'
                            ) {
                                captions_generated =
                                    ' (' + this.paella.translate('automatically generated') + ')';
                            }
                            if (
                                tag.startsWith('type:') &&
                                tag.substring('type:'.length) === 'closed-caption'
                            ) {
                                captions_closed = '[CC] ';
                            }
                        });
                    ...
                    if (captions_lang) {
                        ...
                        const captions_language_name =
                            languageNames.of(captions_lang) ??
                            this.paella.translate('Unknown language');
                        captions_description =
                            captions_closed + captions_language_name + captions_generated;
                    }
    ...
    getFrameList(event: Event): Manifest['frameList'] {
        ...
        const segmentPreviewAttachmentsFlavours = this.conversionConfig
            .segmentPreviewAttachmentsFlavours ?? ['presentation/segment+preview'];
    ...
    getPreviewUrl(event: Event): string {
        ...
        const playerPreviewAttachmentsFlavours = this.conversionConfig
    ...
    getStreams(event: Event): Manifest['streams'] {
        ...
        const flavoredTracks = tracks
            .filter(
                (track) =>
                    ...
                    track.mimetype === 'application/x-mpegURL' ||
                    track.mimetype === 'audio/m4a',
            )
        ...
        const streams = Object.keys(flavoredTracks).map((mainFlavor) => {
            ...
            const hlsSources = tracks
                .filter(
                    (track) =>
                        track.mimetype === 'application/x-mpegURL' && track.is_live === false,
                )
                .map(getSource);
            // getHlsLiveSources(tracks, config);
            const hlsLiveSources = tracks
                .filter(
                    (track) => track.mimetype === 'application/x-mpegURL' && track.is_live === true,
                )
                .map(getSource);
        ...
        const isVideoStream = (stream: Stream): boolean =>
            Boolean(stream.sources.mp4?.length) ||
            Boolean(stream.sources.hls?.length) ||
            Boolean(stream.sources.hlsLive?.length);
    ...
    async getChapters(event: Event): Promise<Manifest['chapters'] | undefined> {
        const tracks = event.tracks ?? [];
        const chatptersFlavours = this.conversionConfig.chaptersFlavours ?? [
            'chapters/source',
            'chapters/delivery',
        ];
