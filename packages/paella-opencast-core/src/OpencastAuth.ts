...
export class OpencastAuthDefaultImpl implements OpencastAuth {
        ...
        try {
            if (this.#userInfo === null) {
                ...
                if (ocPresentationUrl != null) {
                    ...
                    if (url) {
                        this.#player?.log.info(
                            `Fetching opencast user from ${url}`,
                            '@asicupv/paella-opencast-core',
                        );
                        this.#userInfo = await fetch(url).then(
    ...
    async getLoggedUserName(): Promise<string | null> {
        ...
        const isAnonymous =
            userInfo?.roles?.length == 1 && userInfo.roles[0] == userInfo.org?.anonymousRole;
        if (isAnonymous) {
            return null;
        }
    ...
    async canAction(action: string): Promise<boolean> {
        try {
            ...
            this.#player?.log.error(
                'Assuming user cannot write due to error',
                '@asicupv/paella-opencast-core',
            );
            return false;
        }
    ...
    async auth(redirectUrl?: string, redirectTimeoutMs?: number): Promise<void> {
        ...
        if (authUrl) {
            ...
            this.#player.log.info(
                `Redirecting to authentication URL: ${authenticationUrl}`,
                '@asicupv/paella-opencast-core',
            );
