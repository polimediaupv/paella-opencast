...
describe('OpencastPaellaPlayer', () => {
    ...
    describe('getUrlFromOpencastServer method', () => {
        ...
        test('should construct URLs correctly with absolute presentation URL', () => {
            ...
            expect(ocPlayer.getUrlFromOpencastServer('/test')).toBe(
                'https://example.com/opencast/test',
            );
            expect(ocPlayer.getUrlFromOpencastServer('api/endpoint')).toBe(
                'https://example.com/opencast/api/endpoint',
            );
        });
        ...
        test('should handle complex URL paths', () => {
            ...
            expect(ocPlayer.getUrlFromOpencastServer('/search/episode.json?id=123')).toBe(
                'https://api.opencast.org/v1/search/episode.json?id=123',
            );
        });
        ...
        test('should handle localhost URLs with ports', () => {
            ...
            expect(ocPlayer.getUrlFromOpencastServer('/api/info/me.json')).toBe(
                'http://localhost:8080/opencast/api/info/me.json',
            );
        });
    });
    ...
    describe('applyOpencastTheme method', () => {
        test('should apply theme from config successfully', async () => {
            ...
            const getUrlSpy = vi
                ...
                .mockReturnValue('https://example.com/resources/custom-theme/theme.json');
            ...
            expect(loadSkinSpy).toHaveBeenCalledWith(
                'https://example.com/resources/custom-theme/theme.json',
            );
        });
    ...
    describe('Integration tests', () => {
        test('should properly initialize all components together', () => {
            ...
            expect(ocPlayer.getUrlFromOpencastServer('/test')).toBe(
                'https://opencast.example.com/test',
            );
