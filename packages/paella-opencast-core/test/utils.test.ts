...
describe('utils', () => {
    ...
    describe('getUrlFromBase', () => {
        test('should correctly combine base URL and path with proper slashes', () => {
            expect(getUrlFromBase('http://example.com', '/path/to/resource')).toEqual(
        ...
        test('should handle complex URLs with query parameters and fragments', () => {
            expect(getUrlFromBase('https://api.example.com/v1', '/users?active=true')).toEqual(
                'https://api.example.com/v1/users?active=true',
            );
            expect(getUrlFromBase('https://example.com', 'search?q=test#results')).toEqual(
                'https://example.com/search?q=test#results',
            );
        });

        test('should handle URLs with ports and subdomains', () => {
            expect(getUrlFromBase('http://api.subdomain.example.com:8080', '/endpoint')).toEqual(
                'http://api.subdomain.example.com:8080/endpoint',
            );
            expect(getUrlFromBase('https://localhost:3000/', 'api/data')).toEqual(
    ...
    describe('splitFlavor', () => {
        ...
        test('should handle complex MIME types with subtypes', () => {
            expect(splitFlavor('application/vnd.ms-excel')).toEqual([
                'application',
                'vnd.ms-excel',
            ]);
