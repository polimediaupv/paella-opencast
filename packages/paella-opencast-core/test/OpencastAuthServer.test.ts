...
describe('OpencastAuth', () => {
    ...
    test('should always return true for canRead without throwing', async () => {
        const authWithoutPlayer = new OpencastAuthDefaultImpl();
        await expect(authWithoutPlayer.canRead()).resolves.toBe(true);
    ...
    test('should evaluate canAction directly for read and write ACL entries on the same role', async () => {
        const elem: HTMLElement = document.createElement('div');
        const ocPlayer = new OpencastPaellaPlayer(elem, {
            opencast: {
                presentationUrl: '/'
        ...
        const auth = new OpencastAuthDefaultImpl(ocPlayer);
        expect(await auth.canAction('read')).toBe(true);
        expect(await auth.canAction('write')).toBe(true);
    });
    ...
    test('should use absolute auth URL directly when authUrl starts with http', async () => {
        ...
        Object.defineProperty(window, 'location', { writable: true, value: { href: '' } as any });
    ...
    test('should use current window.location.href when auth is called without redirectUrl', async () => {
        const elem: HTMLElement = document.createElement('div');
        const ocPlayer = new OpencastPaellaPlayer(elem, {
            opencast: {
                presentationUrl: '/'
    ...
    test('should redirect when call auth without player initialized', async () => {
        const elem: HTMLElement = document.createElement('div');
        const ocPlayer = new OpencastPaellaPlayer(elem, {
            opencast: {
                presentationUrl: '/'
