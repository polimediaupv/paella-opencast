...
import {
    runButtonPluginTests,
    runPluginOnlyInOpencastTests,
    runPopUpButtonPluginTests,
} from './utils/buttonPluginTests';
...
describe('DescriptionPlugin', () => {
    ...
    describe('applyTemplate()', () => {
        ...
        test('should keep token when variable is not found', () => {
            const result = plugin.applyTemplate('/engage/ui/index.html?q=${missing}', {
                q: 'speaker',
            });
    ...
    describe('getContentTableInfo()', () => {
        ...
        test('should include opencast links in key fields when opencast URL is available', async () => {
            ...
            const contributors = peopleRows.find((r) => r.key === 'Contributor(s)')
                ?.value as string;

            expect(uid).toContain('<a href="?id=test-episode-id">test-episode-id</a>');
            expect(subject).toContain(
                '<a href="/engage/ui/index.html?q=Test Subject">Test Subject</a>',
            );
            expect(series).toContain(
                '<a href="/engage/ui/index.html?epFrom=test-series-id">Test Series Title</a>',
            );
            expect(presenters).toContain('<a href="/engage/ui/index.html?q=John Doe">John Doe</a>');
            expect(contributors).toContain(
                '<a href="/engage/ui/index.html?q=Contributor 1">Contributor 1</a>',
            );
        });
    });
});
