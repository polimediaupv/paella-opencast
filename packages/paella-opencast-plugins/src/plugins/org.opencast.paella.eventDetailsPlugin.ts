...
export default class OpencastEventDetailsPlugin extends TableInfoPopUpPlugin<OpencastEventDetailsPluginConfig> {
    ...
    async getContentTableInfo(): Promise<ContentTableInfo> {
        ...
        const hasOpencastUrl =
            (this.player as OpencastPaellaPlayer).opencastPresentationUrl != null;
        ...
        const presenters = metadata?.presenters
            ?.map((p) =>
                hasOpencastUrl
                    ? `<a href="${this.applyTemplate(engageQueryUrl, { q: p })}">${p}</a>`
                    : p,
            )
            ?.join(', ');
        ...
        const language = metadata?.language
            ? (new Intl.DisplayNames([metadata.language], { type: 'language' }).of(
                  metadata.language,
              ) ?? '')
            : '';
            ...
            table: [
                ...
                },
                {
                    ...
                    rows: [
                        ...
                        {
                            key: 'Start date',
                            value: metadata?.startDate?.toLocaleDateString() ?? '',
                        },
                        {
                            key: 'Duration',
                            value: metadata?.duration ? secondsToTime(metadata.duration) : '',
                        },
