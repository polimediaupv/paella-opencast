...
export default class OpencastRelatedDocumentsDataPlugin extends DataPlugin<
    OpencastRelatedDocumentsDataPluginConfig,
    RelatedDocuments
> {
    ...
    get context() {
        return this.config.context ?? ['file.content'];
    }
            ...
            const filesData = await Promise.all(
                files.map(async (fileInfo) => {
                    ...
                    if (f_att) {
                        ...
                        const content = await fetch(url).then(async (response) => {
                            if (!response.ok) {
                                return null;
                            } else {
