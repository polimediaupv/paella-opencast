...
export default class OpencastDownloadsPlugin extends MenuGroupButtonPlugin {
    ...
    constructor(player: Paella, name: string) {
        ...
        this._eventConversor = new EventConversor(
            player,
            ocConfig.opencast?.conversionConfig ?? {},
        );
    }
    ...
    async getHelp() {
        return {
            ...
            description: this.player.translate(
                'This plugin allows users to download the video tracks / attachments of the video.',
            ),
        };
    }
    ...
    async isEnabled() {
        ...
        const hasWritePermission: boolean =
            !!this.config.enableOnWritePermission && (await this.player.opencastAuth.canWrite());
        const hasAllowedLicense: boolean =
            this.config.enableOnLicenses && metadata?.license
                ? this.config.enableOnLicenses.includes(metadata.license)
                : true;
        const enabled: boolean =
            (await super.isEnabled()) && (hasWritePermission || hasAllowedLicense);
    ...
    filterElements(
        elements: TrackOrAttachment[],
        filterConfig: OpencastDownloadsFilterElementsConfig = {},
    ): TrackOrAttachment[] {
        return elements.filter((elem) => {
            if (filterConfig.downloadFlavors) {
                const downloadFlavors = filterConfig.downloadFlavors?.includes(elem.flavor);
                if (downloadFlavors === undefined || !downloadFlavors) {
                    return false;
                }
            }
            ...
            if (filterConfig.downloadTags) {
                ...
                if (downloadTags === undefined || !downloadTags) {
                    return false;
                }
            }
            ...
            if (filterConfig.downloadMimeTypes) {
                ...
                if (downloadMimeTypes === undefined || !downloadMimeTypes) {
                    return false;
                }
            }
    ...
    convertToDownloadableContent(
        items: TrackOrAttachment[],
        _groupByFlavor: boolean = true,
    ): MenuGroupItem[] {
        // if (!groupByFlavor) {
    ...
    async getDownloadableContent(): Promise<MenuGroupItem[]> {
        ...
        const downloadableAttachments = this.convertToDownloadableContent(
            filteredAttachments,
            false,
        ); //this.config.attachments?.groupByFlavor ?? true);
