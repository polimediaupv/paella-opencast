...
export default class OpencastRelatedVideosDataPlugin extends DataPlugin<
    OpencastRelatedVideosDataPluginConfig,
    RelatedVideosDataResponse
> {
    ...
    async isEnabled() {
        ...
        const ocPlayer = this.player;
        if (ocPlayer.opencastPresentationUrl == null) {
            this.player.log.warn(
                `${this.name} is not enabled because Opencast presentation URL is not set`,
                `${this.getPluginModuleInstance().moduleName} [${this.name}]`,
            );
            return false;
        }
    ...
    get context() {
        return this.config.context || ['relatedVideos'];
    }
    ...
    async read(_context: string, _key: string): Promise<RelatedVideosDataResponse> {
        ...
        try {
            ...
            const eventsRes = await this.getCommonEventsFromSearchAPI({
    ...
    async getCommonEventsFromSearchAPI({
        episodeId,
        series,
        limit,
    }: { episodeId?: string; series?: string; limit?: number } = {}): Promise<Event[]> {
        const ocPlayer = this.player as OpencastPaellaPlayer;
        const url = ocPlayer.getUrlFromOpencastServer(
            `/search/episode.json?id=${episodeId ?? ''}&sid=${series ?? ''}&limit=${limit ?? 100}&sort=created%20desc`,
        );
        if (url === null) {
            this.player.log.warn(
        ...
        const events = await fetch(url)
            ...
            .then((data) => {
                if (data['search-results'] !== undefined) {
                    ...
                    return ensureArray(data['search-results'].result).map((e) =>
                        opencastSearchResultToOpencastPaellaEvent(e),
                    );
                }
    ...
    getPreview(event: Event): string {
        ...
        const playerPreviewAttachmentsFlavours = this.config.playerPreviewAttachmentsFlavours ?? [
